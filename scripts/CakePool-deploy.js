...
const { abi:abiCToken, bytecode:bytecodeCToken } = require('../artifacts/contracts/MasterChef.sol/CakeToken.json');
const { abi:abiSyrupBar, bytecode:bytecodeSyrupBar } = require('../artifacts/contracts/MasterChef.sol/SyrupBar.json');
const { abi:abiMasterChef, bytecode:bytecodeMasterChef } = require('../artifacts/contracts/MasterChef.sol/MasterChef.json');
...
async function main() {
  ...
  const TokenTemp = await new ethers.ContractFactory(abiTemp, bytecodeCToken, owner);
  const tokenTemp = await TokenTemp.attach(token.address);
  var amount = ethers.utils.parseUnits('10000000000', 18);
  let tx = await tokenTemp.mint(amount);
  await tx.wait();
